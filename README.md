...
- **Webhook mode (optional, off by default).** `MODE = "webhook"` receives push events the moment they happen, instead of discovering them on the next poll. Every incoming request is authenticated against an `X-Hub-Signature-256` HMAC of your app secret; unsigned or mis-signed requests are rejected. Webhooks also carry the postback payload, so a button tap is distinguishable from a typed reply — polling can't see that.
  - Three things must line up or events silently never arrive: the `WEBHOOK_VERIFY_TOKEN` secret, the callback URL registered in the Meta dashboard, and the **account** subscribed to the app. chatmany does that last one for you when you connect Instagram; `GET /admin/webhook` reports whether it actually took.
  - **Polling doesn't stop, it slows down.** Webhooks are fire-and-forget: Meta retries a delivery a few times and then drops it, so a redeploy or a brief error loses those leads with nothing to notice. A reconciliation poll every 15 minutes (`WEBHOOK_RECONCILE_SECONDS`) re-reads recent comments and messages and picks up anything missed. Running both is safe because the idempotency ledgers recognise already-handled events and skip them. Set it to `"off"` for pure push.
  - Until all three are in place, leave `MODE = "polling"`. Turning webhook mode on before the Meta side is ready means no events arrive at all and only the 15-minute reconciliation sweep runs, which presents as chatmany being mysteriously slow rather than as a setup error.
