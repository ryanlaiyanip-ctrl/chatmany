...
- **Webhook mode (default).** `MODE = "webhook"` receives push events the moment they happen, instead of discovering them on the next poll. Every incoming request is authenticated against an `X-Hub-Signature-256` HMAC of your app secret; unsigned or mis-signed requests are rejected. Webhooks also carry the postback payload, so a button tap is distinguishable from a typed reply — polling can't see that.
  ...
  - Switching back to `MODE = "polling"` needs no other change, and requires no Meta setup at all.
